...
################################################################################
##################### Workshop on R for Data manipulaiton ##########################
#####################    Day 1 :  April 06, 2023          ##########################  
################################################################################
...
#Challange: Can you use R to calculate 10 to the power of 100? What does that notation appear to mean? 
#10^100

#Challange: Convert "5.001" to a double, and then an integer. What happens?

as.integer(as.double("5.001"))

#Challange: See if 1 is greater than 1.5, and seperately if 2 is greater than 1.5, and mulitply both answers?
#What do you think is happening? 

(1>1.5)*(2>1.5)

#Challange: what datatypes shown here can you not convert between?
as.integer("f")
...
#If I want to assign a value to a variable name to use for later, such as my_fav_num <- 7 (think of it as a little arrow), I can assign it to a variable
my_fav_num<- 7
my_fav_num
...
my_math = function(x){
  return(as.character(x^2 + 3))
}
my_math(2)

#challange: write a function which takes in value x and uses paste to return using paste() 
#"Figure x"

figure_namer = function(x){
  return(paste("Figure",x))
  
}
figure_namer(3)

#challange: write a functions that take in integers x1 and x2 and returns
...
#Challange: Can you create 1,2,2,3,3,3,4,4,4,4 using the rep function? 
...
#challange: Access numbers 1,2,3 from challange_vec using indexing
challange_vec<-c(rep(0,3),c(1,2,3),rep(0,3))
print(challange_vec)

challange_vec[c(4,5,6)]
...
#Challange: Create a dataframe using myvec1 and myvec2 and myvec 3. 
myvec1 = c(5,6,7)
myvec2 = c(8,9,10)
myvec3 = c(11,12,13)
...
#can also perform conditional indexing using boolean statments!
...
#say I mislabled savings to be negative, but want it to have a floor at 0
...
#Saving data 
setwd("C:/Users/David/OneDrive/Desktop/Backup FIles/CISER/Workshop")
...
my_frame = data.frame()
for(i in 1:3){
  my_frame = rbind(my_frame, rep(i, times = 6))
}
my_frame
